...
// vNoZoom-1: abrir imagen ampliada sin zoom extra (ajustada a pantalla) y centrada
(function(){
  function ensureModal(){
    var m = document.getElementById('zoom-modal');
    if(!m){
  ...
  function centerModal(){
    ...
    // limpiar tamaños inline que otros scripts pudieran haber puesto
    img.style.width=''; img.style.height=''; img.style.maxWidth='95vw'; img.style.maxHeight='95vh';
    cont.scrollLeft = 0; cont.scrollTop = 0;
    requestAnimationFrame(function(){
      requestAnimationFrame(function(){
        var sw = img.scrollWidth || img.clientWidth;
  ...
  function openNoZoom(ev){
    var target = ev.target.closest('#lb-img'); if(!target) return;
    ev.preventDefault(); ev.stopPropagation();
    var src = getMainSrc(target, ev.target); if(!src) return;
    var m = ensureModal(); var img = m.querySelector('.zm-img');
    // evitar zoom: quitar cualquier tamaño inline cuando cargue
    function onLoadOnce(){ img.removeEventListener('load', onLoadOnce); img.style.width=''; img.style.height=''; centerModal(); }
    img.addEventListener('load', onLoadOnce);
    img.src = src;
    m.classList.add('active'); document.body.classList.add('no-scroll');
    if (img.complete) onLoadOnce();
    // Observador que limpia si otro script intenta aplicar tamaños inline
    if (!img.dataset.nozoomObs){
      var obs = new MutationObserver(function(muts){
        muts.forEach(function(m){ if(m.attributeName==='style'){ img.style.width=''; img.style.height=''; } });
      });
      obs.observe(img, { attributes:true, attributeFilter:['style'] });
      img.dataset.nozoomObs = '1';
    }
  }
  function bindDelegated(){
    if (document.body.dataset.noZoomBound==='1') return;
    document.addEventListener('click', openNoZoom, true);
    window.addEventListener('resize', centerModal);
    window.addEventListener('orientationchange', centerModal);
    if (window.visualViewport){ window.visualViewport.addEventListener('resize', centerModal); }
    document.body.dataset.noZoomBound='1';
  }
