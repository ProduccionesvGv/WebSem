...
const DATA_OVERRIDE = {
  "02Genext": {
    "01": {"title":"Dealer Deal XXL","genetica":"Autofloreciente","price":"65.000","oldPrice":"80.000"},
    "02": {"title":"Psycho XXL","genetica":"Autofloreciente","price":"65.000","oldPrice":"80.000"},
...
function buildCarousel(rootId, folder){
  ...
  ['01','02','03'].forEach(id=>{
    ...
    // Tarjetas del carrusel: mantener el formato estándar (título + genética + precio)
        const isSale = !!(meta.oldPrice && meta.price);
        const priceHtml = meta.price
          ? (isSale
              ? '<span class="price-old">$' + meta.oldPrice + '</span><span class="price-new">$' + meta.price + '</span>'
              : '$' + meta.price)
          : 'Consultar';
        body.innerHTML = `<h3>${meta.title}</h3><div class="spec">${meta.genetica}</div><div class="price${isSale ? ' price--sale' : ''}">${priceHtml}</div>`;
